	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			try
			{
				...
				{
					TestingOnly = FileActionTesting.Contains(action);
					if (TestingOnly)
					{
		...
		{
			var r = new Regex(@"\s-\sCopy\.", RegexOptions.None);
			...
			{
				ConsoleAndLogged(
					"Nothing to do: the file's root name does not end with \" - Copy\"",
					"RemoveCopyMark",
					!TestingOnly ? 10 : NoDelay,
					!TestingOnly ? LogToFile : LogToConsole);
				if (!TestingOnly)
				{
					return;
				}
			...
			var newFilename = Path.Combine(
					...
					testRootName.Substring(0, testRootName.Length - r.Match(testRootName).Length) + Path.GetExtension(Filename)
			);

			if (File.Exists(newFilename))
			{
				ConsoleAndLogged(
					$"Cannot remove copy mark because the original file still exists",
					"RemoveCopyMark",
					!TestingOnly ? 10 : NoDelay,
					!TestingOnly ? LogToFile : LogToConsole);
				if (!TestingOnly)
